...
const Routes = ({ store }: { store: MobileStore }) => {
  ...
  return <main className="app-shell"><TopBar title="Segments & loops" back={() => setScreen('home')} /><section className="page-heading"><RouteIcon className="heading-icon" /><h1>{routes.length} routes</h1><p>Detected by shared TypeScript analysis over the normalized local archive.</p></section><section className="analysis-action"><div><strong>{settings?.analyzedAt ? `Last rebuilt ${new Date(settings.analyzedAt).toLocaleString()}` : 'Analysis has not been built yet'}</strong><small>Rebuild replaces derived matches only. It does not import recorder-only iPhone workouts or change source workouts.</small></div><Button disabled={rebuilding} onClick={() => void rebuild()}><RefreshCw /> {rebuilding ? 'Rebuilding…' : 'Rebuild analysis'}</Button></section>{notice && <p className="notice notice-error">{notice}</p>}<section className="route-list">{routes.map((route) => <button key={route.id} onClick={() => void open(route.id)} disabled={pending(requests, 'route-detail')}><RouteGeometryMap points={route.geometry} /><div><span className={`route-kind route-kind-${route.type}`}>{route.type === 'loop' ? <Repeat2 /> : <RouteIcon />}{route.type}</span><h2>{route.name}</h2><p>{distance(route.distanceM)} · {route.workoutCount} workouts · {route.traversalCount} traversals</p></div></button>)}</section></main>
}
...
const RideMap = ({ trail, quality, showTiles = true, fitRoute = false }: { trail: readonly RecorderLocationObservation[]; quality: AvailableSessionSnapshot['metrics']['locationQuality']; showTiles?: boolean; fitRoute?: boolean }) => {
  ...
  return <div className={`ride-map${fitted ? ' ride-map-fitted' : ''}`} aria-label={`${fitRoute ? 'Complete recorded route' : 'Live geographic map'} with ${trail.length.toLocaleString()} GPS points`}>

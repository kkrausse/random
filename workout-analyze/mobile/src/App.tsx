...
const analysisStatusText = (status: MobileState['analysisStatus']) => {
  if (status.state === 'completed') return `Completed: ${status.routes ?? 0} routes and ${status.traversals ?? 0} traversals from ${status.activities ?? 0} workouts in ${duration(status.durationMs ?? 0)}.`
  if (status.state === 'error') return `Analysis failed${status.durationMs === null ? '' : ` after ${duration(status.durationMs)}`}: ${status.error ?? 'Unknown error'}`
  if (status.phase === 'load-inputs') return status.activities === null ? 'Loading normalized workouts and GPS samples…' : `Loaded ${status.activities.toLocaleString()} workouts and ${(status.samples ?? 0).toLocaleString()} GPS samples.`
  if (status.phase === 'prepare-paths') return `Preparing workout paths · ${(status.completed ?? 0).toLocaleString()} of ${(status.total ?? 0).toLocaleString()} workouts`
  if (status.phase === 'find-candidates') return `Finding repeated paths · ${(status.completed ?? 0).toLocaleString()} of ${(status.total ?? 0).toLocaleString()} path scans · ${(status.candidates ?? 0).toLocaleString()} candidates`
  if (status.phase === 'match-candidates') return `Matching candidates · ${(status.completed ?? 0).toLocaleString()} of ${(status.total ?? 0).toLocaleString()} · ${(status.qualified ?? 0).toLocaleString()} qualified`
  if (status.phase === 'build-results') return `Building route results · ${(status.completed ?? 0).toLocaleString()} of ${(status.total ?? 0).toLocaleString()} · ${(status.routes ?? 0).toLocaleString()} routes`
  if (status.phase === 'publish-results') return 'Publishing routes and traversals atomically…'
  return 'Ready to analyze the normalized workout library.'
}
...
const Home = ({ store }: { store: MobileStore }) => {
  ...
  const localHost = useStore(store, (state) => state.localBrowserHost)
  ...
  const importedIphone = useStore(store, (state) => state.importedIphoneWorkouts)
  ...
  return <main className="app-shell home-screen">
    ...
    <section className="archive-entries"><button onClick={() => setScreen('library')}><Database /><span><strong>Workout library</strong><small>{analysisAvailable ? `${libraryCount} normalized workouts${importedIphone === null || archiveState === 'unavailable' ? '' : ` · ${importedIphone} of ${savedWorkouts.length} saved iPhone workouts imported`}` : 'Analysis archive unavailable'}</small></span><ChevronRight /></button><button onClick={() => setScreen('routes')}><RouteIcon /><span><strong>Segments & loops</strong><small>{analysisAvailable ? routeCount ? `${routeCount} detected routes` : 'Loading analysis…' : 'Analysis archive unavailable'}</small></span><ChevronRight /></button></section>
    {!analysisAvailable && !localHost && bridge.phase === 'ready' && <p className="capability-note">Native analysis archive unavailable. Saved iPhone workouts remain available.</p>}
...
export const Library = ({ store }: { store: MobileStore }) => {
  const analysisAvailable = useStore(store, (state) => state.analysisHostAvailable)
  ...
  const importFromMac = useStore(store, (state) => state.importCanonicalArchiveFromMac)
  const macSource = useStore(store, (state) => state.macArchiveSourceDraft)
  const setMacSource = useStore(store, (state) => state.setMacArchiveSourceDraft)
  const progress = useStore(store, (state) => state.archiveTransferProgress)
  const imported = useStore(store, (state) => state.archiveImport)
  const iphoneIngestion = useStore(store, (state) => state.iphoneIngestion)
  const importedIphone = useStore(store, (state) => state.importedIphoneWorkouts)
  const savedCount = useStore(store, (state) => state.savedWorkouts.length)
  const archiveState = useStore(store, (state) => state.archiveLoadState)
  const importSavedIphone = useStore(store, (state) => state.importSavedIphoneWorkouts)
  ...
  if (!analysisAvailable) return <main className="app-shell"><TopBar title="Workout library" back={() => setScreen('home')} /><section className="page-heading"><Database className="heading-icon" /><h1>Workout library</h1></section><p className="notice notice-error" role="alert">The native analysis database could not open. Saved iPhone recordings are still available under Saved iPhone workouts. Imports need the analysis database to be available.</p><section className="analysis-action"><div><strong>Saved iPhone recordings</strong><small>Import saved rides into the normalized workout library.</small></div><Button disabled><Upload /> Import saved iPhone workouts</Button></section><section className="analysis-action"><div><strong>Portable canonical archive</strong><small>Import normalized workout history from the Mac or Files.</small></div>{nativeImport ? <><Button disabled><Wifi /> Import from Mac</Button><Button disabled><Upload /> Import legacy ZIP from Files</Button></> : <Button disabled><Download /> Download archive</Button>}</section></main>
  return <main className="app-shell"><TopBar title="Workout library" back={() => setScreen('home')} /><section className="page-heading"><Database className="heading-icon" /><h1>{items.length} workouts</h1><p>Normalized local DuckDB archive. Imports do not run segment analysis.</p></section><section className="analysis-action"><div><strong>Saved iPhone recordings</strong><small>{archiveState === 'error' ? 'Saved recording archive could not load.' : importedIphone === null ? 'Checking saved iPhone recordings…' : `${importedIphone} of ${savedCount} saved iPhone workouts imported`}</small><small>Saved recordings sync automatically when the analysis database is available. Segment analysis still needs a separate rebuild.</small>{iphoneIngestion && <small>Latest sync: {iphoneIngestion.imported} imported · {iphoneIngestion.unchanged} unchanged</small>}</div><Button disabled={pending(requests, 'iphone-import') || pending(requests, 'archive-list')} onClick={() => void importSavedIphone()}><Upload /> {pending(requests, 'iphone-import') ? 'Syncing saved workouts…' : 'Sync saved iPhone workouts'}</Button></section><section className="analysis-action"><div><strong>Portable canonical archive</strong><small>{progressText ?? (nativeImport ? 'The web app streams Parquet bytes into native files, then DuckDB imports them. The legacy Files ZIP path remains separate.' : 'Download normalized workouts and samples for manual transfer to iPhone.')}</small>{imported && <small>{imported.inserted} inserted · {imported.unchanged} unchanged · {imported.conflicts.length} conflicts</small>}</div>{nativeImport ? <><label className="archive-source"><span>Local Mac URL</span><input aria-label="Local Mac URL" type="url" value={macSource} onChange={(event) => setMacSource(event.target.value)} autoCapitalize="none" autoCorrect="off" spellCheck={false} /></label><Button disabled={transferBusy || !macSource.trim()} onClick={() => void importFromMac()}><Wifi /> {pending(requests, 'archive-transfer-mac') ? 'Importing Parquet…' : 'Import from Mac'}</Button><Button disabled={transferBusy} onClick={() => void importArchive()}><Upload /> Import legacy ZIP from Files</Button></> : <Button disabled={transferBusy || items.length === 0} onClick={() => void exportArchive()}><Download /> {transferBusy ? 'Exporting…' : 'Download archive'}</Button>}</section>{imported?.analysisRebuildRequired && <p className="notice">Canonical history is imported. Open Segments &amp; loops and tap Rebuild analysis; derived Mac analysis was intentionally not transferred.</p>}{pending(requests, 'library-list') && items.length === 0 && <p className="notice">Loading workout history…</p>}{notice && <p className={requests['archive-transfer-import']?.status === 'error' || requests['archive-transfer-mac']?.status === 'error' || requests['iphone-import']?.status === 'error' ? 'notice notice-error' : 'notice'}>{notice}</p>}<section className="history-list">{items.map((item) => <button key={item.id} onClick={() => void open(item.id)} disabled={pending(requests, 'library-detail')}><Bike /><span><strong>{new Date(item.startedAt).toLocaleDateString()} · {item.sport}</strong><small>{item.distanceM === null ? 'Distance unavailable' : distance(item.distanceM)} · {item.durationSeconds === null ? 'Duration unavailable' : duration(item.durationSeconds * 1000)}</small></span><ChevronRight /></button>)}</section></main>
}
...
export const Routes = ({ store }: { store: MobileStore }) => {
  const analysisAvailable = useStore(store, (state) => state.analysisHostAvailable)
  ...
  if (!analysisAvailable) return <main className="app-shell"><TopBar title="Segments & loops" back={() => setScreen('home')} /><section className="page-heading"><RouteIcon className="heading-icon" /><h1>Segments &amp; loops</h1></section><p className="notice notice-error" role="alert">The native analysis database could not open. Segment results and rebuild will be available when its store can open.</p><section className="analysis-action"><div><strong>Segment analysis unavailable</strong><small>Saved workouts and the recorder archive have not been changed.</small></div><Button disabled><RefreshCw /> Rebuild segment analysis</Button></section></main>
  return <main className="app-shell"><TopBar title="Segments & loops" back={() => setScreen('home')} /><section className="page-heading"><RouteIcon className="heading-icon" /><h1>{routes.length} routes</h1><p>Analysis reads only the normalized local archive. Import workouts separately in Workout library.</p></section><section className="analysis-action"><div><strong>{settings?.analyzedAt ? `Last rebuilt ${new Date(settings.analyzedAt).toLocaleString()}` : 'Analysis has not been built yet'}</strong><small>{analysisStatusText(analysisStatus)}</small>{ingestion && <small>Latest iPhone import: {ingestion.imported} imported · {ingestion.unchanged} unchanged.</small>}</div><Button disabled={rebuilding} onClick={() => void rebuild()}><RefreshCw /> {rebuilding ? 'Running segment analysis…' : settings?.analyzedAt ? 'Rebuild segment analysis' : 'Run segment analysis'}</Button></section>{analysisStatus.state === 'error' && <p className="notice notice-error" role="alert">Retry is safe; existing published analysis was left unchanged.</p>}{notice && requests['analysis-rebuild']?.status === 'error' && <p className="notice notice-error">{notice}</p>}<section className="route-list">{routes.map((route) => <button key={route.id} onClick={() => void open(route.id)} disabled={pending(requests, 'route-detail')}><RouteGeometryMap points={route.geometry} interactive={false} /><div><span className={`route-kind route-kind-${route.type}`}>{route.type === 'loop' ? <Repeat2 /> : <RouteIcon />}{route.type}</span><h2>{route.name}</h2><p>{distance(route.distanceM)} · {route.workoutCount} workouts · {route.traversalCount} traversals</p></div></button>)}</section></main>
}
...
const RideMap = ({ trail, quality, showTiles = true, fitRoute = false }: { trail: readonly RecorderLocationObservation[]; quality: AvailableSessionSnapshot['metrics']['locationQuality']; showTiles?: boolean; fitRoute?: boolean }) => {
  ...
  const tiles = !fitted && center ? [-1, 0, 1].flatMap((dy) => [-1, 0, 1].map((dx) => ({ href: `https://server.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/${zoom}/${Math.floor(center.y / 256) + dy}/${Math.floor(center.x / 256) + dx}`, x: (Math.floor(center.x / 256) + dx) * 256 - center.x + 160, y: (Math.floor(center.y / 256) + dy) * 256 - center.y + 110 }))) : fitted?.tiles ?? []
  ...
  return <MapViewport className={`ride-map map-bleed${fitted ? ' ride-map-fitted' : ''}`} label={`${fitRoute ? 'Complete recorded route' : 'Live geographic map'} with ${trail.length.toLocaleString()} GPS points`} overlay={<><Pill tone={quality === 'good' ? 'good' : 'warning'}>{quality} GPS</Pill>{showTiles && <small className="attribution">Tiles © Esri, contributors</small>}</>}>
    <svg viewBox="0 0 320 220" preserveAspectRatio="none" aria-hidden="true">{showTiles && tiles.map((tile) => <image key={`${tile.href}-${tile.x}-${tile.y}`} href={tile.href} x={tile.x} y={tile.y} width="256" height="256" />)}{fitted ? <><path className="trail-shadow" d={fitted.path} /><path className="trail-line" d={fitted.path} /><circle className="route-start" cx={fitted.start.x} cy={fitted.start.y} r="4" /></> : path && <><polyline className="trail-shadow" points={path} /><polyline className="trail-line" points={path} /></>} {rider && <><circle className="rider-ring" cx={rider.x} cy={rider.y} r="13" /><circle className="rider" cx={rider.x} cy={rider.y} r="6" /></>}</svg>

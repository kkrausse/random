...
import type { AppBuildStatus, DiagnosticCheckResult, DiagnosticSnapshot, HeartRateMeasurement, HeartRateStatus, LocationObservation, LocationProbeStatus, NativeEvent, PermissionStatus, StatusRow } from '../../src/shared/mobile'
...
const Diagnostics = ({ client, bridge, onUtilities }: { client: BridgeClient; bridge: BridgeState; onUtilities: () => void }) => {
  ...
  return <main className="app-shell diagnostics-screen">
    ...
    <SensorHarness client={client} evidence={bridge.transport === 'native' ? 'Real native device data' : 'Simulated fixture evidence'} />
...
const SensorHarness = ({ client, evidence }: { client: BridgeClient; evidence: string }) => {
  const [permissions, setPermissions] = useState<PermissionStatus | null>(null)
  ...
  const refresh = useCallback(async () => {
    const [nextPermissions, nextLocation, nextHeartRate] = await Promise.all([client.request('permissions.status', {}), client.request('location.status', {}), client.request('heartRate.status', {})])
    setPermissions(nextPermissions); setLocation(nextLocation); setHeartRate(nextHeartRate)
  }, [client])
  useEffect(() => {
  ...
  return <section className="sensor-harness">
    ...
    <article className="probe-card">
      <div className="probe-title"><MapPin /><div><strong>Core Location</strong><small>{location?.reason ?? 'Reading status…'}</small></div><Pill tone={location?.state === 'active' ? 'good' : 'neutral'}>{location?.state ?? 'loading'}</Pill></div>

...
- Domain buttons call the centralized Zustand store actions. Developer scripts use the explicit `app.actions` facade over those exact same functions; do not duplicate a button workflow with direct bridge commands when testing that workflow. Keep lower-level `bridge.request` available only for protocol diagnostics.

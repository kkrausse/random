/* Plain JavaScriptCore phase-1 fallback artifact. */
(function (root) {
  'use strict'
    ...
    return Object.freeze({
      processBatch: function (batch) {
        ...
        for (var i = 0; i < batch.observations.length; i += 1) {
          ...
          nextSequence = item.sequence; nextTotal += item.value
        }
        lastSequence = nextSequence; total = nextTotal
        return Object.freeze({ algorithmId: ALGORITHM_ID, processedCount: batch.observations.length, lastSequence: lastSequence, total: total, displayValue: total })
      },

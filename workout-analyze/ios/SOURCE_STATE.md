...
At startup, if the original store fails to open, the shell tries `analysis-native.duckdb`, then previously created `analysis-recovery-<timestamp>-<uuid>.duckdb` stores (newest first). If none opens, it creates a new uniquely named recovery store with the pinned native DuckDB. It does not import or rebuild data into that store automatically. A missing database with a stranded `.wal` is skipped rather than recreated at the same basename. The existing databases, WALs, and independent `recording-v1.sqlite` journal remain in place. The `analysisDatabase` diagnostics row and database log entries record each failed opening and the selected filename; availability enables `database.query` and the other database bridge capabilities.

...
Physical-device runtime/performance verification is still required. Versioned Mac-to-iPhone normalized archive transfer is now implemented as a canonical-only ZIP bundle. The shared TypeScript layer owns the manifest, validation, revision hashes, schema, and transactional merge. The native shell only presents Files and serves bounded byte ranges. Imports add absent stable `garmin:` / `iphone:` IDs, skip a previously imported identical revision, and report rather than overwrite any same-ID local workout with unknown or different provenance. Imported changes invalidate derived route tables; the phone rebuilds analysis locally. The recorder SQLite journal and original Garmin archives are outside this operation. Live route recognition is not implemented.

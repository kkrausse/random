...
- **Browser test fixtures (historical):** the in-memory transport remains available to automated tests, but URL selection via `?simulator=1` or `?fault=…` has been removed. Those URLs now load the normal real local host.
...
Any page without a native message handler—including an HTTP development source loaded outside the phone shell—fails closed as **Native bridge unavailable**; it never substitutes fixture sensor evidence. A handler that exists but times out or returns invalid data likewise remains a visible native bridge error. The default development browser separately attaches the real local Bun archive/database host.

...
test("prices each response at its context tier, combines variants, and respects recorded charges", () => {
  const zen = {
  ...
  expect(result.byModel["openai/example"]).toBeCloseTo(1.81)
  expect(Object.keys(result.byModel)).toEqual(["openai/example"])
  expect(result.byBucket[0]).toBeCloseTo(1.31)
  expect(result.byBucket[1]).toBeCloseTo(0.5)
  expect(result.byBucketModel["openai/example"]![0]).toBeCloseTo(1.31)
  expect(result.byBucketModel["openai/example"]![1]).toBeCloseTo(0.5)

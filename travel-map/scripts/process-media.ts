...
import { MEDIA_PROCESSING_VERSION } from "../src/media/process-original";
import { resolveRelativePath } from "../src/storage/paths";

type Arguments = {
	importId?: string;
	limit?: number;
	fixRawOrientation?: boolean;
	concurrency: number;
};
...
async function main() {
	...
	const media = args.fixRawOrientation
		? repository.listReadyRawMediaWithOrientation({
				processingVersion: MEDIA_PROCESSING_VERSION,
				limit: args.limit,
			})
		: repository.listProcessingMedia(args);
	let ready = 0;
	let failed = 0;
	try {
		await mapWithConcurrency(media, args.concurrency, async (item, index) => {
			...
			try {
				...
				failed += 1;
				console.error(
					`[${index + 1}/${media.length}] ${item.media.originalFilename}: ${error instanceof Error ? error.message : "processing failed"}`,
				);
			}
		});
	} finally {
...
function parseArguments(values: string[]): Arguments {
	const result: Arguments = { concurrency: 4 };
	for (let index = 0; index < values.length; index += 1) {
		...
		else if (value === "--fix-raw-orientation") result.fixRawOrientation = true;
		else if (value === "--concurrency") result.concurrency = Number(requiredValue(values, ++index, value));
		else throw new Error(`Unknown argument: ${value}`);
	}
	if (result.fixRawOrientation && result.importId)
		throw new Error("--fix-raw-orientation cannot be combined with --import");

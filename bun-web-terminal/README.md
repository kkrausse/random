...
- Mouse clicks and drags go to applications that request mouse tracking, allowing their buttons and application-owned selection to work. Hold **Shift** while dragging to force local text highlighting, then use **Cmd+C** to copy; the highlight stays after release. Without mouse tracking, ordinary dragging also highlights locally. Since tmux enables mouse tracking even at the shell, use **Shift+drag** there too. Auto-copy is disabled; toggle `copyOnSelect` in `src/client.ts` to enable it. Scrolling still goes through tmux.

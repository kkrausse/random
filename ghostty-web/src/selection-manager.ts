...
export class SelectionManager {
  ...
  private restoreContextMenu: (() => void) | null = null;
  private contextMenuTimer: ReturnType<typeof setTimeout> | null = null;
  ...
  private shouldSelect(event: MouseEvent): boolean {
    return (
      event.shiftKey ||
      (this.terminal.options.selectOnDrag && !event.altKey) ||
  ...
  private attachEventListeners(): void {
    ...
    this.boundContextMenuHandler = (e: MouseEvent) => {
      // A second menu must restore the first borrow before saving the caret's
      // styles. Otherwise its dismissal permanently restores a fixed textarea.
      this.restoreContextMenu?.();
      // Position textarea at mouse cursor
      const previousStyle = this.textarea.style.cssText;
      ...
      const resetTextarea = () => {
        if (this.contextMenuTimer !== null) clearTimeout(this.contextMenuTimer);
        this.contextMenuTimer = null;
        this.textarea.style.cssText = previousStyle;
        this.textarea.value = '';
        ...
        this.restoreContextMenu = null;
      };
      this.restoreContextMenu = resetTextarea;
    ...
    if (navigator.clipboard && typeof ClipboardItem !== 'undefined') {
      try {
        ...
        navigator.clipboard.write([clipboardItem]).then(
          () => {
            this.terminal.options.onClipboardWrite(true);
          },
          (err) => {
  ...
  private copyWithExecCommand(text: string): void {
    ...
    const previousStyle = this.textarea.style.cssText;
    const previousValue = this.textarea.value;
    try {
      ...
      this.terminal.options.onClipboardWrite(success);
    } catch (err) {
      console.warn('execCommand copy threw:', err);
      this.terminal.options.onClipboardWrite(false);

...
/**
 * Stable public bridge key codes. official-key.ts maps the historical numeric
 * values to the pinned official C enum (which includes an additional FN key).
 */
export enum Key {
  // Unidentified key
  UNIDENTIFIED = 0,
 ...
 */
export type GhosttyWasmExports = import('./official-abi').OfficialExports;
...
/**
 * Host-side cursor state (not a C struct layout).
 */
...
/**
 * Host-side terminal configuration, applied with ghostty_terminal_set.
 * Colors use 0xRRGGBB. Omitted colors and zero retain Ghostty's defaults,
 * preserving the historical bridge sentinel used by terminal.ts.
 */
...
/**
 * Host-side renderer cell, decoded from the official packed GhosttyCell.
 */
export interface GhosttyCell {
  ...
  hyperlink_id: number; // 0 = no link, 1 = link present; resolve URI by position
  grapheme_len: number; // number of extra codepoints beyond first (not limited to u8)
}

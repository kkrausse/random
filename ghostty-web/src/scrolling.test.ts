...
describe('Terminal Scrolling', () => {
  ...
  describe('Alternate Screen Mode', () => {
    ...
    test('should send one arrow for a small nonzero wheel delta', async () => {
      const dataSent: string[] = [];
      terminal.onData((data) => dataSent.push(data));
      ...
      expect(dataSent).toEqual(['\x1B[A']);
    });

    test('should only send a mouse report when mouse tracking is active', async () => {
      const dataSent: string[] = [];
      terminal.onData((data) => dataSent.push(data));
      ...
      expect(dataSent).toEqual(['\x1B[<64;1;1M']);
    });
  ...
  describe('Edge Cases', () => {
    ...
    test('should preserve a scrolled viewport when output arrives', async () => {
      for (let i = 0; i < 30; i++) terminal.write(`Line ${i}\r\n`);
      terminal.scrollLines(-5);
      const before = terminal.getViewportY();

      terminal.write('More output\r\n');

      expect(terminal.getViewportY()).toBeGreaterThanOrEqual(before);
    ...
    test('should handle very small deltaY values', async () => {
      ...
      // A nonzero trackpad gesture must not disappear after delta conversion.
      expect(dataSent).toEqual(['\x1B[A']);
    });

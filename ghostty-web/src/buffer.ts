/**
 * Read-only xterm-style buffer access (partial compatibility).
 *
 ...
 * Provides named wrappers for normal and alternate screens. The bridge exposes
 * only the active screen: inactive wrappers have length 0 and no readable lines
 * or cursor/viewport state. They are not snapshots of the inactive screen.
 */
...
export class Buffer implements IBuffer {
  ...
  get viewportY(): number {
    ...
    // Terminal scroll offset counts back from the bottom; public buffer rows
    // count forward from the oldest retained line. Match renderer rounding.
    return base - Math.max(0, Math.min(base, Math.floor(this.terminal.getViewportY())));
  }

  get baseY(): number {
    return this.bufferType === 'normal' ? (this.getWasmTerm()?.getScrollbackLength() ?? 0) : 0;
  }
  ...
  getLine(y: number): IBufferLine | undefined {
    ...
    // Check bounds
    if (!Number.isInteger(y) || y < 0 || y >= this.length) {
      return undefined;
    }
    ...
    // Resolve graphemes now, alongside the cell snapshot: a lazy WASM lookup
    // would read a different row after scrolling, writing, or switching screens.
    const history = this.bufferType === 'normal' && y < scrollbackLength;
    const graphemes = cells.map((cell, col) => cell.width !== 0 && cell.grapheme_len > 0
      ? history
        ? wasmTerm.getScrollbackGraphemeString(y, col)
        : wasmTerm.getGraphemeString(lineNumber, col)
      : undefined);
    return new BufferLine(cells, isWrapped, wasmTerm.cols, graphemes);
  }
  ...
  private getWasmTerm(): GhosttyTerminal | undefined {
    ...
    if (!wasmTerm || wasmTerm.isAlternateScreen() !== (this.bufferType === 'alternate')) {
      return undefined;
    }
    return wasmTerm;
  }
...
export class BufferLine implements IBufferLine {
  ...
  constructor(cells: GhosttyCell[], isWrapped: boolean, length: number,
    private graphemes: (string | undefined)[] = []) {
    this.cells = cells;
  ...
  getCell(x: number): IBufferCell | undefined {
    if (!Number.isInteger(x) || x < 0 || x >= this._length) {
      return undefined;
    }
    ...
    return new BufferCell(this.cells[x], x, this.graphemes[x]);
  }
...
export class BufferCell implements IBufferCell {
  ...
  constructor(cell: GhosttyCell, x: number, private grapheme?: string) {
    this.cell = cell;
    this.x = x;
  }

  getChars(): string {
    // Wide-character spacer cells never contribute text, even if the engine
    // stores a sentinel codepoint in them.
    if (this.cell.width === 0) return '';
    if (this.grapheme !== undefined) return this.grapheme;
  ...
  getFgColorMode(): number {
    // Local API convention: -1 denotes resolved RGB. The bridge does not
    // preserve the original default/palette color mode in GhosttyCell.
    return -1;
  }

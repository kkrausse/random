 ...
 *
 * - Commits IME composition with beforeinput de-duplication
 * - Preserves browser shortcuts and delegates selection-aware copying
 */
...
export interface MouseTrackingConfig {
  /** Prefer local selection unless Alt is held. Wheel reporting is unaffected. */
...
export class InputHandler {
  ...
  private localSelectionGesture = false;
  ...
  private attach(): void {
    ...
    this.container.addEventListener('blur', this.clearPressedKeys, true);
  ...
  private handleKeyDown(event: KeyboardEvent): void {
    ...
    // Unhandled Command shortcuts belong to the browser. Cmd+C/V are handled
    // above so terminal selection and native paste keep working.
    if (event.metaKey) return;

    // Virtual/layout keyboards may produce text without a known physical key.
    if (this.mapKeyCode(event.code) === null && this.isPrintableCharacter(event)) {
      event.preventDefault();
  ...
  private encodeKey(event: KeyboardEvent, action: KeyAction): void {
    // Native encoding owns terminal modes, including Kitty keyboard reporting.
    ...
    try {
      ...
      // Preserve the browser's layout/shift result, including non-ASCII text.
      const utf8 = [...event.key].length === 1 ? event.key : undefined;

      const encoded = this.encoder.encode({
        action,
        key,
        // Retain the existing AltGr text policy: Ctrl+Alt producing a character
        // is layout text, not an Alt-prefixed control character.
        mods: utf8 && event.ctrlKey && event.altKey ? mods & ~(Mods.CTRL | Mods.ALT) : mods,
        utf8,
        // Browsers expose the produced character, not an unshifted layout map.
        // Case-fold letters for Kitty's base codepoint without altering utf8.
      ...
      if (action !== KeyAction.RELEASE || data.length > 0) {
        event.preventDefault();
        event.stopPropagation();
      }
      if (action !== KeyAction.RELEASE) this.pressedKeys.add(event.code);

      // Emit the data
      if (data.length > 0) {
        this.onDataCallback(data);
        if (action !== KeyAction.RELEASE) this.recordKeyDownData(utf8 ?? data);
      }
    } catch (error) {
  ...
  private handleCompositionUpdate(_event: CompositionEvent): void {
    if (this.isDisposed) return;
    // ImeOverlay owns preedit presentation; only compositionend emits data.
  }
  ...
  private handleMouseDown(event: MouseEvent): void {
    if (this.isDisposed) return;
    this.localSelectionGesture =
      event.button === 0 &&
      (event.shiftKey || (!!this.mouseConfig?.preferSelection?.() && !event.altKey));
    if (this.localSelectionGesture) return;
  ...
  private handleMouseUp(event: MouseEvent): void {
    if (this.isDisposed) return;
    if (event.button === 0 && this.localSelectionGesture) {
      this.localSelectionGesture = false;
      return;
    }
    if (!this.mouseConfig?.hasMouseTracking()) return;
  ...
  private handleMouseMove(event: MouseEvent): void {
    ...
    if (
      this.localSelectionGesture ||
      event.shiftKey ||
      (this.mouseConfig?.preferSelection?.() && !event.altKey && this.mouseButtonsPressed === 0)
    )
      return;
    if (!this.mouseConfig?.hasMouseTracking()) return;

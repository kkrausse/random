...
describe('Buffer Access API', () => {
  ...
  test('background colors are correctly restored after alternate screen exit', async () => {
    ...
    expect(altViewport![0].bg_b).toBeGreaterThan(0); // Blue need not contain red.
...
describe('Options Proxy handleOptionChange', () => {
  ...
  test('keyboard input makes a blinking cursor visible', async () => {
    if (!container) return;

    const term = await createIsolatedTerminal({ cursorBlink: true });
    term.open(container);
    const renderer = term.renderer!;
    // @ts-ignore - force the hidden blink phase for the interaction test
    ...
    // @ts-ignore - verify input reset the renderer's blink phase

 ...
 *
 * Provides an xterm-style browser API wrapping Ghostty's WASM terminal emulator.
 * Only the interfaces implemented here are supported; compatibility is partial.
 *
 * Usage:
 * ```typescript
...
export class Terminal implements ITerminalCore {
  ...
  private handleFontChange(): void {
    ...
    // Resize canvas to match new font metrics. renderer.resize() owns ALL
    // canvas sizing: it sets the devicePixelRatio-scaled backing store, the
    // CSS size, and the context scale. Writing canvas.width here without the
    // ratio would clobber that to a 1x backing (blurry on retina) and reset
  ...
  open(parent: HTMLElement): void {
    ...
    try {
      ...
      if (getComputedStyle(parent).position === 'static' || !getComputedStyle(parent).position) {
        this.originalPosition = parent.style.position;
      ...
      // Keep a real caret box for the platform IME candidate-window anchor.
      ...
      // Create renderer (Canvas2D unless rendererType opts into WebGL)
      ...
      this.renderer.resize(this.cols, this.rows);
      this.imeOverlay = new ImeOverlay(this.textarea, () => this.updateImePosition());
      ...
      const mouseConfig: MouseTrackingConfig = {
        preferSelection: () => this.options.selectOnDrag,
      ...
      // InputHandler registers first so mouse-aware applications receive their
      // report before this listener handles untracked wheel fallback behavior.
      parent.addEventListener('wheel', this.handleWheel, { passive: false });
  ...
  private writeInternal(data: string | Uint8Array, callback?: () => void): void {
    ...
    const wasScrolled = this.viewportY > 0;
    const previousScrollbackLength = wasScrolled ? this.getScrollbackLength() : 0;
    ...
    // Preserve history while scrollback grows. This cannot detect eviction at
    // a fixed history capacity or track reflow; those require official tracked
    // anchors from the bridge rather than estimating scrolls from PTY text.
    if (wasScrolled) {
      const scrollbackLength = this.getScrollbackLength();
      const growth = Math.max(0, scrollbackLength - previousScrollbackLength);
      if (growth > 0) {
        this.viewportY = Math.min(scrollbackLength, this.viewportY + growth);
  ...
  resize(cols: number, rows: number): void {
    ...
    try {
      ...
      // Resize renderer. It owns ALL canvas sizing (devicePixelRatio-scaled
      // backing store, CSS size, context scale) — writing canvas.width here
      // without the ratio clobbered it to a 1x backing (blurry on retina).
  ...
  /** Keep the browser's IME caret box aligned with the rendered cursor. */
  private updateImePosition(): void {
    if (!this.wasmTerm || !this.renderer || !this.canvas) return;
    const cursor = this.wasmTerm.getCursor();
    this.imeOverlay?.position({
      left: this.canvas.offsetLeft + cursor.x * this.renderer.charWidth,
      top: this.canvas.offsetTop + cursor.y * this.renderer.charHeight,
      width: this.renderer.charWidth,
      height: this.renderer.charHeight,
      fontFamily: this.options.fontFamily,
      fontSize: this.options.fontSize,
      foreground: this.options.theme?.foreground ?? DEFAULT_THEME.foreground,
      background: this.options.theme?.background ?? DEFAULT_THEME.background,
      visible: this.viewportY === 0 && cursor.visible,
    });
  }

  /** Focus terminal input. */
  focus(): void {
    if (this.isOpen) this.forwardFocus();
  }
  ...
  private handleWheel = (e: WheelEvent): void => {
    ...
    // InputHandler reports the wheel as a mouse button when tracking is active.
    // Do not also scroll history or synthesize cursor keys for the same event.
    ...
    if (isAltScreen) {
      ...
      const count = Math.min(Math.max(1, Math.abs(Math.round(e.deltaY / 33))), 5); // Cap at 5

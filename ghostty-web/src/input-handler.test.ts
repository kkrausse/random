...
async function nativeInput() {
  ...
  return {
    ...
    key(code: string, key: string, options: KeyboardEventInit = {}, type = 'keydown') {
      const event = new KeyboardEvent(type, { code, key, bubbles: true, cancelable: true, ...options });
      textarea.dispatchEvent(event);
      return event;
    },
    dispose() { input.dispose(); terminal.free(); },
  };
}

describe('native terminal keyboard mode transitions', () => {
  test('syncs application cursor and backarrow on every key', async () => {
    const h = await nativeInput();
    try {
      h.key('ArrowUp', 'ArrowUp');
      h.terminal.write('\x1b[?1h');
      h.key('ArrowUp', 'ArrowUp');
      h.terminal.write('\x1b[?1l');
      h.key('ArrowUp', 'ArrowUp');
      h.key('Backspace', 'Backspace');
      h.terminal.write('\x1b[?67h');
      h.key('Backspace', 'Backspace');
      h.terminal.write('\x1b[?67l');
      h.key('Backspace', 'Backspace');
      expect(h.data).toEqual(['\x1b[A', '\x1bOA', '\x1b[A', '\x7f', '\x08', '\x7f']);
    } finally { h.dispose(); }
  });

  test('Kitty disambiguates Escape and reports printable press, repeat, release only when enabled', async () => {
    const h = await nativeInput();
    try {
      h.key('Escape', 'Escape');
      ...
      h.key('KeyA', 'a');
      h.key('KeyA', 'a', {}, 'keyup');
      expect(h.data).toEqual(['\x1b', '\x1b[27u', 'a']);
      h.terminal.write('\x1b[<u\x1b[>11u'); // disambiguate + events + report all
      h.data.length = 0;
      h.key('KeyA', 'a');
      h.key('KeyA', 'a', { repeat: true });
      h.key('KeyA', 'a', {}, 'keyup');
      expect(h.data).toEqual(['\x1b[97u', '\x1b[97;1:2u', '\x1b[97;1:3u']);
  ...
  test('Kitty preserves browser shortcuts and suppresses duplicate beforeinput text', async () => {
    const h = await nativeInput();
    try {
      h.terminal.write('\x1b[>11u');
      for (const code of ['KeyC', 'KeyV', 'KeyR', 'KeyL']) {
        expect(h.key(code, code.slice(-1).toLowerCase(), { metaKey: true }).defaultPrevented).toBe(false);
        expect(h.key(code, code.slice(-1).toLowerCase(), {}, 'keyup').defaultPrevented).toBe(false);
      }
      ...
      expect(h.data).toEqual(['\x1b[97u']);
      h.textarea.dispatchEvent(new Event('blur'));
      h.key('KeyA', 'a', {}, 'keyup');
      expect(h.data).toEqual(['\x1b[97u']);
    } finally { h.dispose(); }
  });

  test('preserves shifted layout text and non-ASCII virtual-keyboard fallback', async () => {
    const h = await nativeInput();
    try {
      h.key('KeyA', 'A', { shiftKey: true });
      h.key('Digit1', '!', { shiftKey: true });
      h.key('KeyE', 'é');
      h.key('KeyQ', '@', { ctrlKey: true, altKey: true });
      h.key('Unidentified', '😀');
      expect(h.data).toEqual(['A', '!', 'é', '@', '😀']);
    } finally { h.dispose(); }
  });
});
...
describe('InputHandler', () => {
  ...
  describe('Constructor and Lifecycle', () => {
    test('local drag suppresses mouse reports but Alt-drag and wheel still reach tmux', () => {
      ...
      const mouse = { button: 0, clientX: 20, clientY: 20, altKey: false, shiftKey: false };
      const input = handler as any;
      input.handleMouseDown(mouse);
      input.handleMouseMove({ ...mouse, clientX: 80 });
      input.handleMouseUp(mouse);
      input.handleMouseMove(mouse);
      expect(dataReceived).toEqual([]);
      input.handleMouseDown({ ...mouse, altKey: true });
      input.handleMouseMove({ ...mouse, altKey: true });
      input.handleMouseUp({ ...mouse, altKey: true });
  ...
  describe('Control Characters', () => {
    ...
    test('allows unhandled Cmd shortcuts to reach the browser', () => {
      const handler = new InputHandler(
        ghostty,
        container as any,
        (data) => dataReceived.push(data),
      ...
      const event = createKeyEvent('KeyL', 'l', { meta: true });

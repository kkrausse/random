...
- Marking a parent inactive also marks its loaded subagents and nested descendants inactive, interrupting each running child or child with known pending requests first. Selection advances past the affected family. Restoring a session restores that session only.

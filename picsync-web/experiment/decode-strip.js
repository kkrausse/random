...
export async function decodeStrip(createModule, { bytes, halfSize, count, index }, stage = () => {}) {
  let raw;
  try {
    const started = performance.now();
    stage("Initialize LibRaw WASM");
    const module = await createModule();
    stage("Create LibRaw decoder");
    ...
    stage("Open RAW original");
    raw.open(bytes, settings);
    stage("Read RAW metadata");

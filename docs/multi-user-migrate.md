...
- [x] Create `src/app/user/[username]/trips/page.tsx` — User Trips. Mirrors current `src/app/trips/page.tsx` but filters trips by `userId`.
...
- [x] Update `src/app/sighting/[id]/edit/EditForm.tsx`: change the "Back" link to `/sighting/[id]` (per the new diagram, edit returns to detail rather than Home).
- [x] Update post-save / post-delete redirect targets in `SightingForm.tsx` (and any caller): after save → `/sighting/[id]` or `/user/[me]`; after delete → `/user/[me]`.

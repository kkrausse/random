...
In chat history, reading prompts show the question and highlighted passage in a compact card, with the full page context available from `View page context`. OpenCode responses render as Markdown and update directly from the authenticated `/api/event` stream. If the volatile stream disconnects, the extension reconnects and reloads canonical messages to recover missed events. A compact footer shows the session's input, output, reasoning, cache read/write, and USD cost totals reported by OpenCode. New messages, streamed response text, and new inline highlights automatically scroll into view. Wheel, touch, keyboard, or scrollbar input pauses auto-scroll for five seconds so the reader's position is not overridden; manually reaching the bottom immediately pins the chat to continuing updates again. The persistent header remains available while prompts and responses are loading, so `New`, `Chats`, and `Settings` can be opened at any time without discarding the current capture. If a conversation is open, a new selection keeps it open and offers the configured quick questions plus a custom question. Appending sends only the new highlight and question; `Start new chat with page context` opens the normal new-chat form with the full capture. From other screens, selecting text opens `New`, including when the capture came from another tab.

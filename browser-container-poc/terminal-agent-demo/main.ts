...
document.querySelector<HTMLButtonElement>('#download')!.onclick = async () => {
  ...
  const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([JSON.stringify(report, null, 2)], { type: 'application/json' })); a.download = 'terminal-agent-demo-diagnostics.json'; a.click(); setTimeout(() => URL.revokeObjectURL(a.href), 1000);
};

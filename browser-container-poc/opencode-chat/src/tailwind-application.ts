...
export function readTailwindWasmCandidate(receiptPath: string, expectedReceiptSha256: string) {
  ...
  const pinFile = new URL('../../vivari/tailwind-wasm-candidate.json', import.meta.url);
  assert.equal(receipt.recipe.pinSha256, sha256(readFileSync(pinFile)), 'Candidate pin file changed');

...
async function main() {
  ...
  if (!await Bun.file(`${runtimeRoot}/distribution.json`).exists()) {
    ...
    catch { throw Error("Build the pinned runtime first. From browser-container-poc run: bun vivari/scripts/build-runtime.ts patched && bun workspace-api/scripts/distribution.ts. See editable-app-demo/README.md for prerequisites."); }
  ...
  try {
    ...
    if (existing?.name === "editable-app-demo" && existing.root === root && existing.runtimeRoot === runtimeRoot && existing.preparedRoot === preparedRoot) {

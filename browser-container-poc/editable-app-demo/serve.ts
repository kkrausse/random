...
import { authorizeEditorRequest } from "@kev-browser-agent-kit/workspace/server";
...
function serve() { try { return Bun.serve({
  ...
  async fetch(request) {
    ...
    try {
      ...
      if (path === "/demo-info") return Response.json({ name: "editable-app-demo", root, runtimeRoot, preparedRoot, localEditorAdmin: process.env.LOCAL_EDITOR_ADMIN === "1" }, { headers });
      if (path === "/setup-check") {
    ...
    } catch (error) { if (path !== "/diagnostics") record("http.failed", { path, elapsedMs: Math.round(performance.now() - started), error }); return new Response("Request failed; see editable-app-demo/.diagnostics/events.jsonl", { status: 500, headers }); }
  },
}); } catch (error) {

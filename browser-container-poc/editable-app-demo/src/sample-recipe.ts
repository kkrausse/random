import type { Distribution, Runtime, Workspace, Endpoint } from "@kev-browser-agent-kit/workspace";
  ...
  return {
    async setup(signal?: AbortSignal) {
      ...
      if (!response.ok) {
        ...
        throw Error("Dev server is outdated. Restart bun run demo in editable-app-demo and open the printed URL");
      }
      manifest = await loadPrepared("/prepared/", signal);
      const responseRuntime = await fetch("/runtime/distribution.json", { signal });
      if (!responseRuntime.ok) throw Error("Runtime manifest missing. From editable-app-demo run bun run demo");

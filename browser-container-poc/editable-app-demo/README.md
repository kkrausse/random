...
From the repository root, after the package setup below:

```sh
LOCAL_EDITOR_ADMIN=1 bun run --cwd browser-container-poc/editable-app-demo demo
```

Default URL: **http://127.0.0.1:4311**.
...
bun run --cwd editable-app-demo demo
```
...
- `@kev-browser-agent-kit/workspace/react`: generic controlled lifecycle and provider.
...
**`browser-container-poc/editable-app-demo/.diagnostics/events.jsonl`**, with the
...
For live inspection from `editable-app-demo`:

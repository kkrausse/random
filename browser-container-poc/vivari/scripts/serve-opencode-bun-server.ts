...
const once = process.argv.includes('--once'), runID = crypto.randomUUID()
...
async function report(result: Result) {
  ...
  clearTimeout(timer)
  try {
    await Bun.write(receipt, JSON.stringify({ runID, manifest, result }, null, 2) + '\n')
...
const server = Bun.serve({ hostname: '127.0.0.1', port: Number(process.env.PORT || 0), async fetch(request) {
  ...
  if (path === '/result' && request.method === 'POST') {
    ...
    if (body.result.status === 'PASS' && (body.result.runtime !== runtimeManifest.version || body.result.assets !== assets.length ||
      body.result.exit?.exitCode !== 0 || body.result.exit?.forced !== false || body.result.exit?.signal !== null || body.result.checks?.length !== 6)) {
...
console.log(`OpenCode Bun OPFS: ${server.url}?autorun=1&runID=${runID}`)
if (once) timer = setTimeout(() => { void report({ status: 'FAIL', error: 'Browser qualification timed out after 180 seconds' }) }, 180_000)

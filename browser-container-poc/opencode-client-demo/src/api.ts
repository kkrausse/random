...
export interface NativeEvent { type: string; data: { sessionID?: string; assistantMessageID?: string; ordinal?: number; delta?: string; text?: string; error?: unknown } }
...
export class OpenCodeAPI {
  ...
  async request<T>(path: string, signal?: AbortSignal, body?: unknown): Promise<T> {
    const response = await this.response(path, { signal, ...(body === undefined ? {} : { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }) });
  ...
  async pages<T>(path: string, signal?: AbortSignal): Promise<T[]> {
    ...
    do {
      ...
      if (cursor) { url.searchParams.delete("order"); url.searchParams.set("cursor", cursor); }
      const page = await this.request<Page<T>>(url.pathname.slice(1) + url.search, signal);
      if (!Array.isArray(page.data) || !page.cursor) throw new Error("Invalid V2 paginated response");
  ...
  interrupt(id: string, signal?: AbortSignal) { return this.response(`session/${encodeURIComponent(id)}/interrupt`, { method: "POST", signal }); }
  async events(signal: AbortSignal, ready: () => void, event: (value: NativeEvent) => void) {
    const response = await this.response("event", { signal, headers: { Accept: "text/event-stream" } });
    if (!response.headers.get("content-type")?.includes("text/event-stream") || !response.body) throw new Error("Expected V2 SSE response");
    const reader = response.body.getReader();
    const abort = () => { void reader.cancel().catch(() => {}); };
    signal.addEventListener("abort", abort, { once: true });
    const decoder = new TextDecoder();
    let buffer = "", data: string[] = [], kind = "";
    const line = (text: string) => {
      if (!text) {
        if (data.length) {
          if (kind === "effect/httpapi/stream/failure") throw new Error(`V2 event stream failed: ${data.join("\n")}`);
          const value = JSON.parse(data.join("\n")) as NativeEvent;
          if (typeof value.type !== "string" || !value.data) throw new Error("Invalid V2 event envelope");
    ...
    try {
      ...
      if (!signal.aborted) throw new Error("OpenCode event connection closed. Reconnect to reload history.");
    } finally {

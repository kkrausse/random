...
- A parent shows its loaded sub-agent count beside the title and its running count in the status. Sub-agents are collapsed by default; select the parent and press `Space` (or click its count) to reveal the entire nested family, then repeat to collapse it. Children whose parent is unloaded remain independently selectable until it loads; the current sub-agent and its ancestry remain visible. Search shows matching children even when collapsed. Parent previews still aggregate descendant requests for approval. A root deletion event clears its loaded descendants from the picker immediately.
...
- Plugin startup requests one page of up to 100 recent sessions and resolves missing ancestors of those sessions, so a recent child does not appear orphaned merely because its parent is on an older page. Current and discovered active/attention sessions can also appear outside that page. Older pages load when keyboard or pointer selection approaches the end of the list; wheel scrolling alone does not request another page. Loaded pages survive closing and reopening.
- The plugin-lifetime controller maintains session metadata, statuses, the known-location inbox, and usage previews while the picker is closed. Status and context synchronization each run with four concurrent jobs. Opening and selecting a loaded row read the prepared state without starting fresh requests. Metadata refreshes use a narrow event allowlist; high-volume step, tool, usage, shell, and execution events do not refetch session rows or attention caches. Reconnect reconciles the recent page, active sessions, and attention. Closing only detaches the view; plugin unload cancels background reads.

...
export default function ChecklistClient({ userId, username, displayName }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/sightings?userId=${encodeURIComponent(userId)}`).then((r) => r.json()),
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-1">{displayName}&apos;s Checklist</h1>
      <p className="text-sm text-gray-500 mb-3">
        {seenCodes.size} of {allSpecies.length} species seen &middot;{" "}
        <Link href="/checklist" className="text-blue-600 hover:underline">
          Switch to site-wide list
        </Link>
      </p>
      ...
      <p className="text-xs text-gray-400 mb-2">Showing {filtered.length} species</p>
          ...
          {virtualizer.getVirtualItems().map((virtualItem) => {
            ...
            return (
              <Link
                ...
                href={`/user/${username}/species/${species.speciesCode}`}
                style={{
                  position: "absolute",
                  top: 0,
                  left: 0,
